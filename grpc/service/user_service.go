...
func (u userService) GetById(ctx context.Context, req *user_service.PrimaryKey) (resp *user_service.User, err error) {
	...
	if err != nil {
		u.log.Error("GetUser--->", logger.Error(err))
		return nil, status.Error(codes.InvalidArgument, err.Error())
	}
...
func (u userService) GetAll(ctx context.Context, req *user_service.GetAllUserRequest) (resp *user_service.GetAllUserResponse, err error) {
	u.log.Info("---GetuserList--->", logger.Any("req", req))

	resp, err = u.strg.User().GetList(ctx, req)
	if err != nil {
		u.log.Error("!!!GetUserList--->", logger.Error(err))
		return nil, status.Error(codes.InvalidArgument, err.Error())
	}
...
func (u userService) Update(ctx context.Context, req *user_service.UpdateUserRequest) (resp *empty.Empty, err error) {
	u.log.Info("---UpdateUser--->", logger.Any("req", req))

	resp = &empty.Empty{}

	err = u.strg.User().Update(context.Background(), req)
	fmt.Println("REQ dgldfkgkjfldjgfjgdgjdifjgifdjg", req)
	if err != nil {
